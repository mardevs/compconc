...
// semaforos
sem_t exclusao_mutua; // Semaforo de exclusao mutua.
...
void* le(void* args) {
    ...
    while(1) {
        ...
        sem_wait(&leit);
        sem_wait(&exclusao_mutua); l++;

        if(l == 1) {
            printf("Leitores esperando para ler\n");
            sem_post(&exclusao_mutua); sem_wait(&escr);
            printf("Leitores podem ler\n");
        }
        else { sem_post(&exclusao_mutua); }
...
void* escreve(void* args) {
    ...
    while(1) {
        ...
        sem_wait(&exclusao_mutua); e++;
        
        if(e == 1) {
            printf("Escritores esperando para escrever\n");
            sem_post(&exclusao_mutua); sem_wait(&leit);
